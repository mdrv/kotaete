...
describe('runtime state snapshot helpers', () => {
	test('serializeSnapshot produces valid versioned structure', () => {
		...
		const jobs = new Map<string, unknown>([
			['q-1-1', {
				...
				engine: null,
				meta: {
					sources: ['/tmp/members.json', '/tmp/quiz'],
					groupId: '120@g.us',
					quizDir: '/tmp/quiz',
					membersFile: '/tmp/members.json',
					noCooldown: false,
					noSchedule: false,
					createdAt: now,
					introAt: now,
		...
		const snapshot = __runtimeTestInternals.serializeSnapshot(jobs)
		expect(snapshot.version).toBe(1)
		expect(snapshot.updatedAt).toBeTruthy()
		expect(snapshot.jobs).toHaveLength(1)
		expect(snapshot.jobs[0]!.groupId).toBe('120@g.us')
		expect(snapshot.jobs[0]!.sources).toEqual(['/tmp/members.json', '/tmp/quiz'])
		expect(snapshot.jobs[0]!.quizDir).toBe('/tmp/quiz')
		expect(snapshot.jobs[0]!.membersFile).toBe('/tmp/members.json')
		expect(snapshot.jobs[0]!.noCooldown).toBe(false)
		expect(snapshot.jobs[0]!.noSchedule).toBe(false)
		expect(snapshot.jobs[0]!.noGeneration).toBeUndefined()
		expect(snapshot.jobs[0]!.introAt).toBeTruthy()
		expect(snapshot.jobs[0]!.firstRoundAt).toBeTruthy()
		expect(snapshot.jobs[0]!.createdAt).toBeTruthy()
	})

	test('serializeSnapshot with empty jobs returns empty array', () => {
		const { __runtimeTestInternals } = require('./runtime.ts')
		const jobs = new Map()
		const snapshot = __runtimeTestInternals.serializeSnapshot(jobs)
	...
	test('serializeSnapshot null introAt/firstRoundAt serializes as null', () => {
		const { __runtimeTestInternals } = require('./runtime.ts')
		const now = new Date()
		const jobs = new Map<string, unknown>([
			['q-1-1', {
		...
		const snapshot = __runtimeTestInternals.serializeSnapshot(jobs)
		expect(snapshot.jobs[0]!.introAt).toBeNull()
		expect(snapshot.jobs[0]!.firstRoundAt).toBeNull()
		expect(snapshot.jobs[0]!.noGeneration).toBeUndefined()
	})

	test('serializeSnapshot round-trips through JSON parse', () => {
		const { __runtimeTestInternals } = require('./runtime.ts')
		...
		const jobs = new Map<string, unknown>([
			['q-1-1', {
				...
				engine: null,
				meta: {
					sources: ['/tmp/m.json', '/tmp/quiz'],
					groupId: '120@g.us',
					quizDir: '/tmp/quiz',
					membersFile: '/tmp/m.json',
					noCooldown: false,
					noSchedule: false,
					createdAt: now,
					introAt: now,
					firstRoundAt: new Date('2026-04-03T08:10:00.000Z'),
				},
			}],
			['q-1-2', {
				id: 'q-1-2',
		...
		const entries = __runtimeTestInternals.parseSnapshotEntries(serialized)
		expect(entries).toHaveLength(2)
		expect(entries[0]!.groupId).toBe('120@g.us')
		expect(entries[1]!.groupId).toBe('121@g.us')
		expect(entries[0]!.noCooldown).toBe(false)
		expect(entries[1]!.noCooldown).toBe(true)
		expect(entries[0]!.noGeneration).toBeUndefined()
		expect(entries[1]!.noGeneration).toBeUndefined()
	})

	test('parseSnapshotEntries rejects malformed JSON', () => {
		const { __runtimeTestInternals } = require('./runtime.ts')
		expect(() => __runtimeTestInternals.parseSnapshotEntries('not json')).toThrow()
	})
