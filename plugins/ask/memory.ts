...
export async function ensureMemoryTable(ac: AskContext): Promise<void> {
	...
	await conn.query('DEFINE TABLE OVERWRITE kotaete_ask_memory SCHEMAFULL')
	await conn.query('DEFINE FIELD OVERWRITE lid ON kotaete_ask_memory TYPE string')
	await conn.query('DEFINE FIELD OVERWRITE messages ON kotaete_ask_memory TYPE array')
	await conn.query('DEFINE FIELD OVERWRITE messages[*] ON kotaete_ask_memory TYPE object')
	await conn.query('DEFINE FIELD OVERWRITE messages[*].role ON kotaete_ask_memory TYPE string')
	await conn.query('DEFINE FIELD OVERWRITE messages[*].content ON kotaete_ask_memory TYPE string')
	await conn.query('DEFINE FIELD OVERWRITE messages[*].ts ON kotaete_ask_memory TYPE number')
	await conn.query('DEFINE FIELD OVERWRITE summary ON kotaete_ask_memory TYPE option<string>')
	await conn.query('DEFINE FIELD OVERWRITE created_at ON kotaete_ask_memory TYPE datetime')
	await conn.query('DEFINE FIELD OVERWRITE updated_at ON kotaete_ask_memory TYPE datetime')
	// Per-group memory (shared conversation history across members)
	await conn.query('DEFINE TABLE OVERWRITE kotaete_ask_group_memory SCHEMAFULL')
	await conn.query('DEFINE FIELD OVERWRITE group_id ON kotaete_ask_group_memory TYPE string')
	await conn.query('DEFINE FIELD OVERWRITE messages ON kotaete_ask_group_memory TYPE array')
	await conn.query('DEFINE FIELD OVERWRITE messages[*] ON kotaete_ask_group_memory TYPE object')
	await conn.query('DEFINE FIELD OVERWRITE messages[*].role ON kotaete_ask_group_memory TYPE string')
	await conn.query('DEFINE FIELD OVERWRITE messages[*].content ON kotaete_ask_group_memory TYPE string')
	await conn.query('DEFINE FIELD OVERWRITE messages[*].ts ON kotaete_ask_group_memory TYPE number')
	await conn.query(
		'DEFINE FIELD OVERWRITE messages[*].nickname ON kotaete_ask_group_memory TYPE option<string>',
	)
	await conn.query('DEFINE FIELD OVERWRITE summary ON kotaete_ask_group_memory TYPE option<string>')
	await conn.query('DEFINE FIELD OVERWRITE created_at ON kotaete_ask_group_memory TYPE datetime')
	await conn.query('DEFINE FIELD OVERWRITE updated_at ON kotaete_ask_group_memory TYPE datetime')
}
